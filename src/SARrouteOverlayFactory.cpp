...
#include "SARrouteOverlayFactory.h"
#include <vector>
...
//----------------------------------------------------------------------------------------------------------
//    SARroute Overlay Factory Implementation
//----------------------------------------------------------------------------------------------------------
SARrouteOverlayFactory::SARrouteOverlayFactory(SARrouteUIDialog &dlg)
    : m_dlg(dlg) {
...
SARrouteOverlayFactory::~SARrouteOverlayFactory() {}

void SARrouteOverlayFactory::Reset() {}

void SARrouteOverlayFactory::setData(double lat1, double lon1) {
  myLat1 = lat1;
  myLon1 = lon1;
}

bool SARrouteOverlayFactory::RenderOverlay(piDC &dc, PlugIn_ViewPort &vp) {
  m_dc = &dc;
  if (!dc.GetDC()) {
    if (!glQueried) {
      glQueried = true;
...
void SARrouteOverlayFactory::DrawEBLLineInViewPort(PlugIn_ViewPort *BBox) {
...
void SARrouteOverlayFactory::DrawIndexTargets(PlugIn_ViewPort *BBox) {
  wxColour colour = wxColour("RED");
  wxBrush brush(colour);
  wxPen pen2(colour, 2);
...
void SARrouteOverlayFactory::DrawBearingLineInViewPort(PlugIn_ViewPort *BBox) {
  wxColour colour = wxColour("RED");
  wxBrush brush(colour);
  c_GLcolour = colour;  // for filling GL arrows
  if (m_dc) {
...
void SARrouteOverlayFactory::DrawAllLinesInViewPort(PlugIn_ViewPort *BBox) {
  wxColour colour = wxColour("RED");
  wxBrush brush(colour);
  c_GLcolour = colour;  // for filling GL arrows
  if (m_dc) {
...
wxImage &SARrouteOverlayFactory::DrawGLText(double value, int precision) {
  wxString labels;
...
wxImage &SARrouteOverlayFactory::DrawGLTextDir(double value, int precision) {
  wxString labels;
...
wxImage &SARrouteOverlayFactory::DrawGLTextString(wxString myText) {
  wxString labels;
  labels = myText;
...
wxImage &SARrouteOverlayFactory::DrawGLPolygon() {
  wxString labels;
