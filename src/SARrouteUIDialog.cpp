...
SARrouteUIDialog::SARrouteUIDialog(wxWindow* parent, SARroute_pi* ppi)
    : SARrouteUIDialogBase(parent),
  ...
  this->Connect(wxEVT_MOVE, wxMoveEventHandler(SARrouteUIDialog::OnMove));
...
SARrouteUIDialog::~SARrouteUIDialog() {
  wxFileConfig* pConf = GetOCPNConfigObject();
  ;
...
void SARrouteUIDialog::SetViewPort(PlugIn_ViewPort* vp) {
  if (m_vp == vp) return;
...
void SARrouteUIDialog::OnClose(wxCloseEvent& event) {
  pPlugIn->OnSARrouteDialogClose();
}

void SARrouteUIDialog::OnMove(wxMoveEvent& event) {
  //    Record the dialog position
  wxPoint p = GetPosition();
  pPlugIn->SetSARrouteDialogX(p.x);
  pPlugIn->SetSARrouteDialogY(p.y);

  event.Skip();
}

void SARrouteUIDialog::OnSize(wxSizeEvent& event) {
  //    Record the dialog size
  wxSize p = event.GetSize();
  pPlugIn->SetSARrouteDialogSizeX(p.x);
  pPlugIn->SetSARrouteDialogSizeY(p.y);

  event.Skip();
}

wxString SARrouteUIDialog::MakeDateTimeLabel(wxDateTime myDateTime) {
...
void SARrouteUIDialog::OnInformation(wxCommandEvent& event) {}

void SARrouteUIDialog::GetTable(wxString myRoute) {}

void SARrouteUIDialog::AddChartRoute(wxString myRoute) {}
...
bool SARrouteUIDialog::OpenXML(wxString filename, bool reportfailure) {
  return false;
}
...
/*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
double SARrouteUIDialog::deg2rad(double deg) { return (deg * pi / 180); }
/*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
/*:: This function converts radians to decimal degrees :*/
/*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
double SARrouteUIDialog::rad2deg(double rad) { return (rad * 180 / pi); };
...
void SARrouteUIDialog::AddTestItems(wxCommandEvent& event) {
  wxString filename;
...
void SARrouteUIDialog::AddAreas() {
  int image, imageSel;
...
void SARrouteUIDialog::OnBeginDrag(wxTreeEvent& event) {
  // need to explicitly allow drag
  if (event.GetItem() !=
      pPlugIn->m_pSARrouteDialog->m_treeCtrlRoutes->GetRootItem()) {
...
void SARrouteUIDialog::OnEndDrag(wxTreeEvent& event) {
...
void SARrouteUIDialog::ChartTheRoute(wxString myRoute) {
...
int SARrouteUIDialog::GetScale(double myChartScale) {
...
void SARrouteUIDialog::OnButtonEBL(wxCommandEvent& event) {
  m_bBearingLine = true;

  this->m_timer1.Start(200);
}

void SARrouteUIDialog::OnButtonEBL_off(wxCommandEvent& event) {
  m_bBearingLine = false;
...
void SARrouteUIDialog::key_shortcut(wxKeyEvent& event) {
  // wxMessageBox("here");
...
/*
void SARrouteUIDialog::SetBearingWaypoint() {
 // wxMessageBox("here");
  centreLat = m_vp->clat;
  centreLon = m_vp->clon;
...
void SARrouteUIDialog::MakeBoxPoints() {
...
void SARrouteUIDialog::OnTimer(wxTimerEvent& event) { MakeEBLEvent(); }

void SARrouteUIDialog::MakeEBLEvent() {
  if (m_bBearingLine) {
...
void SARrouteUIDialog::FindIndex(Position* A, Position* B) {
  double value = 0.0;
...
void SARrouteUIDialog::OnRange(wxCommandEvent& event) {
  ReadRTZ();
  active_waypoint = FindActiveWaypoint(id_wpt);
  if (active_waypoint->wpName == wxEmptyString) {
...
void SARrouteUIDialog::FindRange(Position* A, Position* B) {}

void SARrouteUIDialog::OnDirection(wxCommandEvent& event) {
  ReadRTZ();
  active_waypoint = FindActiveWaypoint(id_wpt);
  if (active_waypoint->wpName == wxEmptyString) {
    wxMessageBox("Check NMEA");
...
void SARrouteUIDialog::FindDirection(Position* A, Position* B) {
  double value = 0.0;
  A->lat.ToDouble(&value);
  double lat1 = value;
  A->lon.ToDouble(&value);
...
Position* SARrouteUIDialog::FindActiveWaypoint(wxString wpt_name) {
...
void ConfigurationDialog::OnInformation(wxCommandEvent& event) {
  ...
  */
  pPlugIn->m_pSARrouteDialog->GetTable(rn);
  return;  //
}
...
void ConfigurationDialog::OnGenerate(wxCommandEvent& event) {
  ...
  pPlugIn->m_pSARrouteDialog->AddChartRoute(rn);
}
